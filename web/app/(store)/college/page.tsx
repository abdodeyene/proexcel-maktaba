...
export default async function CollegePage() {
  ...
  try {
    ;[products, categories] = await Promise.all([
      prisma.product.findMany({ where: { niveau: 'College' }, orderBy: { createdAt: 'desc' } }),
  ...
  return (
    <Suspense fallback={<div style={{ textAlign: 'center', padding: '5rem', color: 'var(--text2)' }}>Chargement...</div>}>
      <BestOffersClient
        ...
        pageTitle="Niveau Collège"
        pageSubtitle="Toute la fourniture scolaire pour les élèves du collège"
      />
    </Suspense>
  )
}
