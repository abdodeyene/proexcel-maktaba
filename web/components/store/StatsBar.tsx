  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Stats */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '1rem', width: '100%', paddingTop: '2rem', borderTop: '1px solid rgba(255,255,255,0.07)' }}>
          {STATS.map((stat, i) => (
            <StatCol key={`${stat.label}-${i}`} stat={stat} active={active} delay={i * 140} />
          ))}
        </div>
        ...
        <Link
          ...
          style={{
            ...
            padding: '0.95rem 2.8rem', borderRadius: '12px',
            boxShadow: '0 4px 24px rgba(220,40,40,0.38), inset 0 1px 0 rgba(255,255,255,0.14)',
            textDecoration: 'none',
          }}
        >
          {btnText}
          <span style={{ fontSize: '1.1rem', lineHeight: 1, opacity: 0.85 }}>→</span>
        </Link>
      </div>
